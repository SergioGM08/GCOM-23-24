...
print("Ejemplo gráfico del oscilador simple")
q0 = 0.
dq0 = 1.
...
for i in iseq:
    ...
    plt.title("Ejemplo gráfico del oscilador simple")
#plt.savefig('Time_granularity.png', dpi=250)

print("Ejemplo de coordenadas canónicas (q, p)")
...
print("Ejemplo gráfico de la derivada de q(t)")
fig, ax = plt.subplots(figsize=(12,5))
plt.ylim(-1.5, 1.5)  
plt.rcParams["legend.markerscale"] = 6
ax.set_xlabel("t = n $\delta$", fontsize=12)
...
plt.plot(t, dq, '-')
plt.title("Ejemplo gráfico de la derivada de q(t)")

print("Ejemplo de diagrama de fases (q, p) para una órbita completa")
...
plt.plot(q, p, '-')
plt.title("Ejemplo de diagrama de fases (q, p) para una órbita completa")
plt.show()


print("Ejemplo de diagrama de fases (q, p) para un tiempo determinado")
horiz = 0.251
...
for i in range(len(seq_q0)):
    for j in range(len(seq_dq0)):
        ...
        plt.title("Ejemplo de diagrama de fases (q, p) para un tiempo determinado")
plt.show()
...
def simplectica(q0,dq0,F,col=0,d = 10**(-4),n = int(16/d),marker='-'): 
    ...
    dq = deriv(q,dq0=dq0,d=d)
    p = dq/2
    plt.plot(q, p, marker,c=plt.get_cmap("viridis")(col))
    plt.title("Espacio fásico")


Horiz = 12
d = 10**(-3)
print("No sé qué se plotea aquí")
